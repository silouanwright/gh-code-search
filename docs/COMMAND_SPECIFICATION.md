...
### **Root Command: `gh scout`**
```bash
gh scout <query> [flags]                    # Main search interface
```
...
```bash
gh scout patterns <query> [flags]          # Analyze configuration patterns
gh scout saved <command> [args]            # Manage saved searches
gh scout template <query> [flags]          # Generate templates from patterns
gh scout compare <file1> <file2>           # Compare configurations side-by-side
```

## 📋 **Detailed Command Specifications**

### **1. Main Search Command: `gh scout`**

```bash
Usage: gh scout <query> [flags]
...
Examples:
  ...
  gh scout "eslint.config.js" --language javascript --save eslint-research
  gh scout "next.config.js" --repo vercel/next.js --context 50

  # Advanced filtering for quality results
  gh scout "tailwind.config" --min-stars 1000 --language javascript
  gh scout "package.json" --path "examples/" --limit 20
...
gh scout "config typescript"              # → "config typescript"
gh scout config --language typescript     # → "config language:typescript"
gh scout --filename "*.config.js"         # → "filename:*.config.js"
gh scout react --repo facebook/react      # → "react repo:facebook/react"

# Advanced GitHub search syntax support
gh scout "exact phrase match"             # → "exact phrase match"
gh scout "config AND typescript"          # → "config AND typescript"
gh scout "config NOT test"                # → "config NOT test"
gh scout "extension:js OR extension:ts"   # → "extension:js OR extension:ts"
```

### **2. Pattern Analysis: `gh scout patterns`**

```bash
Usage: gh scout patterns <query> [flags]
...
Examples:
  ...
  gh scout patterns "tsconfig.json" --language json --min-pattern-count 3

  # Find common ESLint configurations
  gh scout patterns "eslint.config" --language javascript --group-by property

  # Analyze Docker patterns with ranking
  gh scout patterns "dockerfile" --rank-by frequency --show-examples 5
...
### **3. Saved Searches: `gh scout saved`**

```bash
Usage: gh scout saved <command> [args]
...
Examples:
  ...
  gh scout saved save "react-configs" "tsconfig.json" --repo "*react*" --language json
  gh scout saved save "docker-examples" "dockerfile" --min-stars 100

  # Use saved searches
  gh scout saved run react-configs
  gh scout saved run docker-examples --limit 20  # Override saved limit

  # Manage saved searches
  gh scout saved list
  gh scout saved edit react-configs
  gh scout saved delete old-search

  # Team sharing
  gh scout saved export team-searches.yaml
  gh scout saved import team-searches.yaml
...
### **4. Template Generation: `gh scout template`**

```bash
Usage: gh scout template <query> [flags]
...
Examples:
  ...
  gh scout template "tsconfig.json" --language json --output tsconfig.template.json

  # Create Docker template from popular examples
  gh scout template "dockerfile" --min-stars 1000 --output Dockerfile.template

  # Generate with pattern analysis
  gh scout template "vite.config" --analyze-patterns --merge-common --output vite.config.template.js
...
### **5. Configuration Comparison: `gh scout compare`**

```bash
Usage: gh scout compare <file1> <file2> [flags]
...
Examples:
  ...
  gh scout compare tsconfig.json ../other-project/tsconfig.json

  # Compare with GitHub examples
  gh scout compare package.json --github facebook/react:package.json

  # Find similar configs for comparison context
  gh scout compare vite.config.js --find-similar --language javascript
...
📊 Check current status: gh scout --rate-limit
⚡ Try this instead: gh scout "config" --repo facebook/react --language json
```
...
📖 Corrected examples:
   gh scout "config typescript"                    # phrase search
   gh scout config --language typescript          # using flags
   gh scout "config AND typescript"               # boolean AND
   gh scout "typescript OR javascript"            # boolean OR

🚀 Try: gh scout "config typescript" --language typescript
```
...
🔍 Alternative searches:
   gh scout "config" --language javascript --limit 10
   gh scout "package.json" --path examples/ --limit 5
   gh scout --saved popular-configs  # if you have saved searches

📖 Browse common patterns: gh scout patterns --help
```
...
🚀 After authentication: gh scout "your query here"
```

## ⚙️ **Configuration System**

### **Config File Format (.gh-scout.yaml)**
```yaml
...
output:
  ...
  save_path: "~/gh-scout-results"  # Where to save result files

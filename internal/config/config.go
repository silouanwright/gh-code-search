...
func (c *Config) Save() error {
	...
	configPath := filepath.Join(configDir, "gh-search.yaml")
...
func getConfigPaths() []string {
	...
	paths := []string{
		".gh-search.yaml",                                    // Current directory
		".gh-search.yml",                                     // Current directory (alternative)
		filepath.Join(configDir, "gh-search.yaml"),          // User config directory
		filepath.Join(configDir, "gh-search.yml"),           // User config directory (alternative)
	}
	
	if homeDir != "" {
		paths = append(paths,
			filepath.Join(homeDir, ".gh-search.yaml"),        // Home directory
			filepath.Join(homeDir, ".gh-search.yml"),         // Home directory (alternative)
		)
	}
...
func getConfigDir() string {
	...
	if configHome := os.Getenv("XDG_CONFIG_HOME"); configHome != "" {
		return filepath.Join(configHome, "gh-search")
	}
	
	if homeDir, err := os.UserHomeDir(); err == nil {
		return filepath.Join(homeDir, ".config", "gh-search")
	}
